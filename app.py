...
# implementing main route for index html
@app.route('/')
def main():
    return (flask.render_template('index.html'))

# implementing route for loan prediction page
@app.route('/loanpred')
def loanprediction():
    return (flask.render_template('loanpred.html'))

# implementation for individual page
...
def individual():
    ...
    if flask.request.method == 'POST':
        ...
        #calculating credit_ratio
        credit_ratio = round(open_account / total_account, 2)

        #calculating credit_history
    ...
    # loading csv file for interset rate
        grade_to_apr_df=pd.read_csv("https://github.com/BHAVI2803/LENDING_CLUB_DEPLOYMENT/blob/ceb3f27fa470fdf71099afc22b9740a9737dec2e/data/grade_to_apr.csv",error_bad_lines=False)
        if term==0:
            ...
            interest_rate = round((grade_to_apr_df[grade_to_apr_df['grade_num'] == sub_grad_value]['60_mo'].values[0]) / 100,2)
            # calculating EMI
        ...
        if dti> 40.0:
            ...
            return render_template('individual.html', pred=f'"OOPS" sorry your loan is denied, your loan risk probabilty is : {round(output*100,2)}% ')
        else:
            return render_template('individual.html', pred=f'"Congratulation",Your loan is approved',cr_ratio=f' your credit ratio is :{credit_ratio}',
                                   apr=f"Applicable apr is : {round(interest_rate*100,2)}%",emi=f"Expected installment /mo : ${emi}",
...
def joint():
    ...
    # we dont have values for columns like grade ,sub grade
        ...
        final_j = scaler.transform(final_j)
        ...
        if dti> 40.0:
            ...
            return render_template('joint.html', pred=f'"OOPS" sorry your loan is denied, your loan risk probabilty is : {round(output_j*100,2)}%')
        else:
